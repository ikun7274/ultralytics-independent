...
| ② 结构 | `slice_all_tiles` | 每图占 4 个槽位（4 片）还是 1 个槽位（随机 1 片）→ base 段宽 `4K_slice` / `K_slice` | 小目标密集 → True |
...
- **稀疏数据不要用 `-1`**：池子会被纯负样本占满。但限空片的代价是槽位变成整图（同图重复出现几次）—— 那是槽位已被 ② 分配后的必然结果，与 B+ 消除的那类可避免重复不同。
